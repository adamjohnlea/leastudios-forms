 ...
 * Nonce helper.
 *
 * @package LEAStudios\Forms\Security
 */

declare(strict_types=1);

namespace LEAStudios\Forms\Security;
...
class Nonce {
	...
	public static function create( string $action ): string {
		return wp_create_nonce( 'leastudios_forms_' . $action );
	}
	...
	public static function verify( string $nonce, string $action ): bool {
		return (bool) wp_verify_nonce( $nonce, 'leastudios_forms_' . $action );
	}
	...
	public static function check_ajax( string $action, string $param_key = '_wpnonce' ): void {
		check_ajax_referer( 'leastudios_forms_' . $action, $param_key );
	}
}
