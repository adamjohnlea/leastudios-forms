 ...
 */
final class Text_Field implements Field_Type {
	...
	public function get_label(): string {
		return __( 'Text', 'leastudios-forms' );
	}
	...
	public function validate( mixed $value, array $field_config ): true|string {
		...
		if ( ! empty( $field_config['validation']['pattern'] ) && '' !== (string) $value ) {
			...
			if ( ! preg_match( $pattern, (string) $value ) ) {
				return sprintf(
					/* translators: %s: field label */
					__( '%s is not in the expected format.', 'leastudios-forms' ),
					$field_config['label'] ?? $field_config['name'] ?? ''
				);
			}
		}
	...
	public function render( array $field_config, mixed $value = null ): string {
		...
		return sprintf(
			'<div class="leastudios-forms-field leastudios-forms-field--text">'
			. '<label for="%1$s">%2$s</label>'
			. '<input type="text" id="%1$s" name="%3$s" value="%4$s" placeholder="%5$s"%6$s />'
			. '</div>',
			$id,
			$label,
			$name,
