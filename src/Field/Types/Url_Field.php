 ...
 * URL input field.
 */
final class Url_Field implements Field_Type {
	...
	public function get_label(): string {
		return __( 'URL', 'leastudios-forms' );
	}
	...
	public function validate( mixed $value, array $field_config ): true|string {
		...
		if ( '' !== (string) $value && false === filter_var( (string) $value, FILTER_VALIDATE_URL ) ) {
			return sprintf(
				/* translators: %s: field label */
				__( '%s must be a valid URL.', 'leastudios-forms' ),
				$label
			);
		}
	...
	public function render( array $field_config, mixed $value = null ): string {
		...
		return sprintf(
			'<div class="leastudios-forms-field leastudios-forms-field--url">'
			. '<label for="%1$s">%2$s</label>'
			. '<input type="url" id="%1$s" name="%3$s" value="%4$s" placeholder="%5$s"%6$s />'
			. '</div>',
			$id,
			$label,
			$name,
